...
students.forEach((student, index) => {
  ...
  test.describe.serial(`Student ${index + 1} - Student Onboarding Flow with Role Application`, () => {
...
// ================= CERTIFICATE =================
  ...
  // ================= ROLES SIDE MENU NAVIGATION =================
  test('Navigate to Roles, Search, Apply and Check Eligibility', async ({ page }) => {

    // Try multiple ways to get the role name due to potential Excel formatting issues
    let roleName = student['Role-Name'] || student['\tRole-Name'];
    
    console.log('Role name from Excel:', roleName);
    console.log('Role name length:', roleName.length);
    console.log('Role name type:', typeof roleName);
    
    // Only use Excel data - no hardcoded fallback
    if (!roleName || roleName.trim() === '') {
      console.log('⚠️ Role name is empty in Excel - test may fail');
      console.log('Available student data keys:', Object.keys(student));
    }

    const rolePage = new RolePage(page);
    const applyPage = new ApplyPage(page);

    console.log('→ Clicking Roles sidebar, searching and applying...');
    await rolePage.clickRolesSearchAndApply(roleName);

    console.log('→ Clicking Check Eligibility button...');
    await applyPage.clickApply();

    console.log('→ Uploading New Resume...');
    const resumeFilePath = './testdata/kumar ramkumar resume (1).pdf';
    await applyPage.uploadNewResume(resumeFilePath);

    console.log('✓ Roles navigation, search, apply and resume upload completed successfully');

...
test('Bulk Upload Login Test', async ({ page }) => {
  const bulkUploadTest = new BulkUploadTest(page);
  ...
  console.log('Login completed successfully through BulkUpload test');
  ...
  // Create StudentsPage instance and click Bulk Upload
  const studentsPage = new StudentsPage(page);
  await studentsPage.verifyStudentsPageVisible();
  await studentsPage.clickBulkUpload();
  
  // Create BulkUploadPage instance and select degree
  const bulkUploadPage = new BulkUploadPage(page);
  await bulkUploadPage.fillDegreeDropdown('Bachelor Of Arts');
 await bulkUploadPage.fillDepartmentDropdown('Arts Of Science');
 
 // Upload file after department selection
 await bulkUploadPage.uploadFile('bulckupload0.xlsx');
 
 // Click Upload button after file selection
 await bulkUploadPage.clickUpload();
  
  // Take screenshot to verify bulk upload page
